...
SELECT
    ...
    CASE 
        WHEN qu.TipoItemAvaliado = 1 THEN 
            -- Se for Turma, buscar da tabela isturmadisc
            CONCAT('Turma: ', COALESCE(td.NomeTurma, 'N/A'), ' - Disciplina: ', COALESCE(td.NomeDisciplina, 'N/A'))
        ELSE 
...
LEFT JOIN
    isturmadisc AS td ON (qu.TipoItemAvaliado = 1 AND r.ItemAvaliadoId = td.Id)
WHERE
    rq.ItemAvaliadoId IS NOT NULL
ORDER BY
    r.DataResposta DESC, p.Nome;

-- Verificar a estrutura da tabela isturmadisc
SELECT 'Estrutura da tabela isturmadisc' AS Status;
DESCRIBE isturmadisc;

-- Verificar dados na tabela isturmadisc
SELECT 'Dados na tabela isturmadisc' AS Status;
SELECT * FROM isturmadisc LIMIT 10;
